...
function regGoStep(n){
  [1,2,3].forEach(i=>{
    ...
    document.getElementById('sd'+i).classList.remove('active','done');
  });
  ...
  for(let i=1;i<n;i++){
    ...
    const sl=document.getElementById('sl'+i); if(sl) sl.classList.add('done');
  }
...
function regStep1(){
  ...
  if(!name||!phone||!parent){showToast('Fill all required fields','error');return;}
  if(!/^\d{10}$/.test(phone)){showToast('Enter valid 10-digit phone number','error');return;}
  regData={name,studentClass:cls,phone,parent,address:document.getElementById('rAddr').value.trim()};
  regGoStep(2);
}
async function regStep2(){
  const user=document.getElementById('rUser').value.trim(),
        pass=document.getElementById('rPass').value,
        secQ=document.getElementById('rSecQ').value,
        secA=document.getElementById('rSecA').value.trim();
  if(!user||pass.length<6||!secA){showToast('Fill all fields; password min 6 chars','error');return;}
  showLoader('Checking username…');
  // Check uniqueness in Firestore
  const allStudents=await load('students');
  const allTeachers=await load('teachers');
  if([...allStudents,...allTeachers].find(u=>u.username===user)){
    hideLoader();showToast('Username already taken — try another','error');return;
  }
  regData={...regData,username:user,password:pass,secQ,secA:secA.toLowerCase()};
  const otp=String(Math.floor(1000+Math.random()*9000));
  sessionStorage.setItem('regOTP',otp);
  document.getElementById('regPhoneDisplay').textContent=regData.phone;
  hideLoader(); regGoStep(3);
  showToast('OTP sent to your registered phone number','info');
}
async function verifyOTPReg(){
  const entered=[1,2,3,4].map(i=>document.getElementById('o'+i).value).join('');
  if(entered!==sessionStorage.getItem('regOTP')){showToast('Incorrect OTP. Try again.','error');return;}
  showLoader('Creating your account…');
  try{
    // Save new student to Firestore
    const newId=await fsAdd('students',{...regData,fee:1500,active:true});
    sessionStorage.removeItem('regOTP');
    hideLoader();
    showToast(`Welcome ${regData.name}! Registration successful 🎉`,'success');
    showAuth('login');
    document.getElementById('loginUser').value=regData.username;
...
function resendOTP(){
  const otp=String(Math.floor(1000+Math.random()*9000));
  sessionStorage.setItem('regOTP',otp);
  showToast('OTP resent to your registered phone number','info');
}
